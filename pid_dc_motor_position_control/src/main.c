...
int main()
{
    ...
    uint32_t t, lastT, switchTime = 0, printTime = 0;
    t = lastT = micros();
    double dt, setpoint, phi, e, integral, derivative, output, lastE;
    setpoint = 50.0;
    phi = lastE = integral = derivative = 0.0;
    bool up = true;
    ...
    while (1)
    {
        ...
        phi = 360.0 * steps / ENCODER_RESOLUTION;

        if (t - switchTime > 5e6)
        {
            switchTime = t;
            setpoint = up ? 90.0 : 0.0;
            up = !up;
        }
        ...
        */
        ...
        {
            printTime = micros();
            printf("Setpoint: %f, phi: %f, Time: %f, output: %f\n", setpoint, phi, US_TO_S(t), output);
        }
